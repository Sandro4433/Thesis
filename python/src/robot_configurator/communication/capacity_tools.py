        ...
        "parameters": {
            ...
            "properties": {
                ...
                "input_containers": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Receptacle names to use as input (pick parts from).",
                },
                "output_receptacles": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Receptacle names to use as output (place parts into).",
                },
                "kit_recipe": {
                    "type": "array",
                    "items": {
                    ...
                    "description": "For kitting: the per-kit recipe. Omit for sorting.",
                },
                "sorting_colors": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "For sorting: list of colors being sorted. Omit for kitting.",
                },
                "batch_size": {
                    "type": "integer",
                    "description": (
                        "For kitting: how many kits to fill. "
...
DESCRIBE_SCENE_TOOL_SCHEMA = {
    ...
    "function": {
        ...
        "description": (
            ...
            "explain, or summarise what is in the scene."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "include_parts": {
                    "type": "boolean",
                    "description": "Whether to include per-slot part details.",
                },
                "focus": {
                    "type": "string",
                    "description": (
                        "Optional: narrow the description to a specific area "
                        "(receptacle name or category 'kits'/'containers')."
                    ),
                },
            },
            "required": [],
...
def execute_capacity_check(args: Dict[str, Any], scene: Dict[str, Any]) -> str:
    """
    Execute the check_capacity tool call.

    Returns a plain-text result the LLM can include in its reasoning.
    """
    ...
    if operation == "kitting" and kit_recipe:
        ...
        num_kits = min(int(batch_size), available_kits) if batch_size is not None else available_kits
        ...
        for rec_name in sorted(output_receptacles & all_known):
            empty = capacity.get(rec_name, {}).get("empty", 0)
            if recipe_per_kit > empty:
                problems.append(
                    f"{rec_name}: recipe needs {recipe_per_kit} slots but only {empty} are empty"
                )
                lines.append(
                    f"  {rec_name} slot check: need {recipe_per_kit}, empty {empty} → INSUFFICIENT"
                )
            else:
                lines.append(
                    f"  {rec_name} slot check: need {recipe_per_kit}, empty {empty} → OK"
                )
        ...
        for color in sorted(colors_to_sort):
            lines.append(f"  {color}: {available.get(color, 0)} parts to sort")
...
def execute_describe_scene(args: Dict[str, Any], scene: Dict[str, Any]) -> str:
    ...
    if focus:
        ...
        if focus_lower == "kits":
            names: List[str] = sorted(n for n in capacity if n.startswith("Kit_"))
    ...
    for name in names:
        ...
        lines.append(
            f"{name} ({pos}{role_str}): {occupied}/{total} slots occupied, {empty} empty"
        )
        if include_parts:
