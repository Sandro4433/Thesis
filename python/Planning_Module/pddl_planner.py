...
def _goal_kitting(state: Dict[str, Any]) -> List[str]:
    ...
    # Build empty_slots lookup early (needed for filtering)
    ...
    # Only consider kits that are marked as output AND have at least one empty slot
    output_kits = sorted(
        [k for k in kit_set if k in outputs and empty_slots.get(k, [])],
        key=rkey
    )
